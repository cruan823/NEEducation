...
智慧云课堂 Android SDK提供了一套简单易用的接口，允许开发者通过调用NEEducation SDK(以下简称SDK)提供的API，快速地集成音视频会议功能至现有Android应用中。
...
2. 添加SDK编译依赖
    ...
    修改工程目录下的'app/build.gradle'文件，添加智慧云课堂SDK的依赖。
    ```groovy
    dependencies {
      //声明SDK依赖，版本可根据实际需要修改
...
1. 获取登录用账号ID和Token。Token由智慧云课堂应用服务器下发，但SDK不提供对应接口获取该信息，需要开发者自己实现。
...
- SDK不提供账号注册机制，第三方应用集成SDK时需要为第三方应用的用户帐号绑定智慧云课堂系统中企业管理员开通的员工帐号，第三方应用的用户帐号和企业员工帐号是1:1映射的。
